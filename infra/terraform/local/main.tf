...
#
# Postgres and Redis run as plain manifests in k8s/base (see postgres.yaml,
# redis.yaml) rather than Terraform-managed Helm releases: on kind they're
# just pods, same as the app. In the GCP-shaped reference module
# (infra/terraform/gcp-reference) the equivalent resources are Cloud SQL and
# Memorystore - real managed services Terraform would own directly, outside
# Kubernetes entirely.
